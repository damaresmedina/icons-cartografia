# PROTOCOLO ONTOLÓGICO UNIVERSAL v7
...
| argumentativa | relação entre fundamentos e decisões | suporta, contradiz |
| fiscal | relação de impacto orçamentário/fiscal | impacta_orcamento, aloca_risco_fiscal |
...
9. extração de argumentos (objects tipo `argumento` + edges `suporta`, `contradiz`, `fundamenta_em`)
10. chunking (segmentação)
11. embeddings (representação semântica)
12. materialização de views
13. publicação (published_objects)
...
→ VIEWS MATERIALIZADAS (slugs resolvidos, domain, estado, árvore)
...
> Ele organiza relações estruturadas entre textos, decisões, normas, instituições e argumentos, em múltiplos domínios, escalas e classes epistêmicas.
