# PROTOCOLO ONTOLÓGICO UNIVERSAL v3
...
| sobre | — | — | vincula object a subject/tema |
...
| `annotations` | event | type: `anotacao_editorial`, com `aggregate_slug` = node alvo |
...
## 8.3 Campos estruturais (no payload do object)
...
Subjects são **objects** do type `subject`. Representam temas, áreas ou assuntos. Vinculam-se a outros objects via edges do tipo `sobre`.

## 9.2 Estrutura

```
object: saude             (type: subject, payload: {area: "direito_fundamental"})
object: tributario        (type: subject, payload: {area: "direito_publico"})
object: meio_ambiente     (type: subject, payload: {area: "direito_difuso"})
object: saude_publica     (type: subject, payload: {area: "politica_publica"})
```
...
```
saude --[parent_of]--> saude_publica
saude --[parent_of]--> saude_suplementar
tributario --[parent_of]--> icms
tributario --[parent_of]--> imposto_renda
```
...
```
ADI-4650 --[sobre]--> financiamento_eleitoral
ADI-4650 --[sobre]--> direitos_politicos
cf-1988-art-196 --[sobre]--> saude
cf-1988-art-196 --[sobre]--> saude_publica
```
...
|---|---|
| obra | `cf-1988` |
| versão | `cf-1988-v-2026` |
| título | `cf-1988-tit-2` |
| capítulo | `cf-1988-tit-2-cap-1` |
...
| decisão | `stf-adi-4650` |
| súmula | `stf-sv-11` |
| subject | `subject-saude` |
| actor | `ministro-barroso` |

## 10.3 Caminho completo (path)
...
→ TAXONOMIAS (object_types · actor_types · event_types · edge_types · role_types · state_types)
