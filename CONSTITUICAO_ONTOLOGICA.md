...
### Documento Mestre · Versão Fechada · Referência Única

---

> **STATUS: CONGELADO**
> Este documento é a constituição do sistema. Toda implementação, schema, migration, pipeline, view ou validação deve ser derivada exclusivamente deste documento. Nenhuma estrutura pode ser criada, alterada ou removida fora do que está aqui definido. Alterações neste documento exigem nova versão constitucional com justificativa e auditoria.
...
**Natureza:** Documento mestre constitucional — referência única e base de validação de toda implementação
**Origem:** Protocolo Ontológico Universal, evoluído em 9 versões de auditoria (v1-v9, 25 de março de 2026)
**Vigência:** Permanente até substituição formal por nova versão constitucional

### Regras constitucionais

1. **Nenhuma implementação pode contradizer este documento.** Se o código divergir da constituição, o código está errado.
2. **Toda nova estrutura deve ser derivável deste documento.** Se não está aqui, não existe.
3. **Alterações exigem nova versão constitucional.** Não se emenda por fora — se emenda por dentro, com número de versão, justificativa e auditoria completa.
4. **Este documento é lido antes de qualquer ação.** Pipeline, schema, migration, view, seed — tudo começa aqui.
...
> Este documento é a constituição ontológica do sistema. Ele não é guia, não é sugestão, não é referência opcional. É a lei fundamental da arquitetura. Toda implementação é medida contra ele.

## Cláusula de congelamento

> A partir da data de assinatura, nenhuma estrutura, tabela, campo, view, pipeline, edge_type, object_type, actor_type, event_type, state_type, domain, provenance, derivado ou validação pode ser criada, alterada ou removida sem que esteja expressamente prevista ou derivável deste documento.

## Cláusula de emenda

> Alterações a este documento constituem nova versão constitucional. Toda emenda exige:
> 1. Identificação do gap ou inconsistência
> 2. Proposta formal de alteração
> 3. Auditoria de impacto sobre implementações existentes
> 4. Aprovação da autora
> 5. Nova versão com numeração sequencial

## Cláusula de precedência

> Em caso de conflito entre qualquer implementação (SQL, código, pipeline, view, API) e este documento, este documento prevalece. O código deve ser corrigido, não o documento.

---

**CONSTITUIÇÃO ONTOLÓGICA — VERSÃO FECHADA**

**Baseada no Protocolo Ontológico Universal v9**
**Congelada em 25 de março de 2026**
